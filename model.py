...
class CISCRec(nn.Module):
    ...
    def fit(self,train_loader):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for user, i_s, uj_s, uj_sp, j, jp, user_prop in pbar:
                if torch.cuda.is_available():
                    user = user.cuda()
                    i_s = i_s.cuda()
                    j = j.cuda()
                    ...
                    uj_sp = uj_sp.cpu()
                    jp = jp.cpu()
                    user_prop = user_prop.cpu()

                self.zero_grad()
                pos_distances, neg_distances, j_bias, jp_bias = self.forward(
                    user,i_s,uj_s,uj_sp,j,jp,user_prop)

                loss = -torch.mean(F.logsigmoid(pos_distances - neg_distances))

                s_bias = torch.squeeze(self.s_biases(torch.LongTensor([0]).cuda()))

                bias = 0
                for item in [j_bias,jp_bias,s_bias]:
                    bias += torch.sum(item**2) / 2

                loss += self.lambda_bias * bias
    ...
    def forward_topk(self,user,i_s,uj_s,j,user_prop):
        real_bsize = user.shape[0]
        user_embed = self.embed_user(user)
        is_embed = self.embed_item(i_s)
        is_embed = torch.mean(is_embed,dim=1)
